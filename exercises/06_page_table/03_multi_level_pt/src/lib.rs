...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in (1..=2).rev() {
            ...
            // 关键：先用不可变借用获取 PTE 的值，避免长时间占用整个 self
        ...
        leaf_node.entries[vpn0] = ((pa >> 12) << PPN_SHIFT) | flags | PTE_V;
    ...
    let node1 = self.nodes.get_mut(&current_ppn).unwrap();
