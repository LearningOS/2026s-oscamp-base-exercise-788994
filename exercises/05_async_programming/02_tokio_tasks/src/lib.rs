...
pub async fn concurrent_squares(n: usize) -> Vec<usize> {
    ...
    // 1. Spawn all tasks concurrently
    for i in 0..n {
    ...
    // 2. Await them sequentially to collect the results
    for handle in handles {
        // handle.await returns a Result<T, JoinError>, so we unwrap it
...
pub async fn parallel_sleep_tasks(n: usize, duration_ms: u64) -> Vec<usize> {
    ...
    // 1. Spawn all "time-consuming" tasks concurrently
    for id in 0..n {
        let handle = tokio::spawn(async move {
    ...
    // 3. Sort the results as requested by the exercise comments
    results.sort();
